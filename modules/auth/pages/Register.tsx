...
import { useSignupMutation } from "@/modules/auth/services/AccountService";
...
const Register: FunctionComponent<RegisterProps> = () => {
    ...
    const [signup, { isLoading, isError, isSuccess, error, data }] = useSignupMutation(); // Destructure to get mutation states
    ...
    useEffect(() => { 
        if (isError) {
            ...
            router.navigate(`(auth)/verify?email:${data.clientId}`)
        }
    }, [isLoading])
        ...
        <View style={testStyles.defaultContainer}>
            ...
            <View  style={{ gap: 10 }}>
                ...
                <CheckboxInput
                    ...
                    onChange={(isChecked) => setFieldValue('receiveNewsLetter', isChecked)}
                    />
                    </View>
            </View>
            <Btn
                title="Sign Up"
                onPress={() => {console.log(errors) ; handleSubmit()}}
                disabled={isLoading}
            />
              <Text >
                    Already a member?
