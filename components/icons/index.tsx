export { default as AmericanExpress } from "./AmericanExpress";
export { default as Arrow } from "./Arrow";
export { default as ArrowBack } from "./ArrowBack";
export { default as Calendar } from "./Calendar";
export { default as Close } from "./Close";
export { default as CloseCircle } from "./CloseCircle";
export { default as Docs } from "./Docs";
export { default as Dot } from "./Dot";
export { default as Download } from "./Download";
export { default as EyeClose } from "./EyeClose";
export { default as FaQs } from "./FaQs";
export { default as Facebook } from "./Facebook";
export { default as GoBack } from "./GoBack";
export { default as Help } from "./Help";
export { default as History } from "./History";
export { default as Home } from "./Home";
export { default as Id } from "./Id";
export { default as Info } from "./Info";
export { default as Instagram } from "./Instagram";
export { default as Languaje } from "./Languaje";
export { default as Location } from "./Location";
export { default as Logout } from "./Logout";
export { default as MarketFood } from "./MarketFood";
export { default as MarketHardwareandtools } from "./MarketHardwareandtools";
export { default as MarketHomeappliances } from "./MarketHomeappliances";
export { default as MarketPersonalcare } from "./MarketPersonalcare";
export { default as Mastercard } from "./Mastercard";
export { default as Menu } from "./Menu";
export { default as Money } from "./Money";
export { default as Password } from "./Password";
export { default as PayPalBig } from "./PayPalBig";
export { default as Paypal } from "./Paypal";
export { default as Phone } from "./Phone";
export { default as PhoneCalling } from "./PhoneCalling";
export { default as Search } from "./Search";
export { default as Settings } from "./Settings";
export { default as ShieldCheck } from "./ShieldCheck";
export { default as ShoppingCart } from "./ShoppingCart";
export { default as Star } from "./Star";
export { default as Tag } from "./Tag";
export { default as Trash } from "./Trash";
export { default as Tuning } from "./Tuning";
export { default as Ufo } from "./Ufo";
export { default as User } from "./User";
export { default as User2 } from "./User2";
export { default as Visa } from "./Visa";
export { default as Visible } from "./Visible";
export { default as WhatsApp } from "./WhatsApp";
export { default as Email } from "./Email";
